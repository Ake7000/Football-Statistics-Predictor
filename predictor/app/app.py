...
/* ---- Compact pitch spacing ---- */
/* Column-level vertical blocks: zero gap — space-evenly (JS) distributes player units */
.st-key-pitch [data-testid="stHorizontalBlock"] > [data-testid="stColumn"] > [data-testid="stVerticalBlock"] {{
    gap: 0px !important;
}}
/* Element containers inside pitch: no extra margin/padding */
.st-key-pitch [data-testid="stElementContainer"] {{
    margin: 0 !important;
    padding: 0 !important;
}}
/* Pitch selectboxes: remove outer margin only */
.st-key-pitch [data-testid="stSelectbox"] {{
    margin: 0 !important;
}}
.st-key-pitch [data-testid="stSelectbox"] [data-baseweb="select"] {{
    font-size: 12px !important;
}}
.st-key-pitch [data-testid="stSelectbox"] [data-baseweb="select"] > div {{
    display: flex !important;
...
def _pitch_col(
    ...
    for i in range(count):
        ...
        with col:
            # Wrap jersey + dropdown in a container so space-evenly treats them as one unit
            with st.container():
...
# The center column is stretch-filled to match side panels by the row's flexbox.
# But the inner wrappers (stLayoutWrapper etc.) don't propagate that height.
# This script sets inline styles with !important on each wrapper to create a
# flex chain from stColumn down to .st-key-pitch, making the pitch fill up.
...
(function(){
  ...
  function fill(){
    ...
    // DOM chain: stColumn > stVerticalBlock > stLayoutWrapper > .st-key-pitch
    let el = pitch;
    const chain = [];
    while(el){
      chain.push(el);
    ...
    // chain[0]=pitch, chain[last]=stColumn
    const col = chain[chain.length - 1];
    if(!col || col.getAttribute('data-testid') !== 'stColumn') return;

    // KEY FIX: The center column's content (pitch with 5-6 players) can be
    // taller than the side panels, pushing the row taller. To prevent this,
    // we use position:absolute on the column's direct child (stVerticalBlock)
    // to remove it from height flow. The row height is then set only by side
    // panels. The center column stretches via align-items:stretch, and the
    // absolute child fills it.
    col.style.setProperty('position', 'relative', 'important');

    // chain[last-1] = stVerticalBlock (direct child of stColumn)
    var directChild = chain[chain.length - 2];
    if(directChild){
      directChild.style.setProperty('position', 'absolute', 'important');
      directChild.style.setProperty('inset', '0', 'important');
      directChild.style.setProperty('display', 'flex', 'important');
      directChild.style.setProperty('flex-direction', 'column', 'important');
    }

    // Every wrapper between directChild and pitch: flex:1 to fill downward
    for(var i = chain.length - 3; i >= 0; i--){
      var e = chain[i];
    ...
    // Pitch itself: clip overflow so tall content doesn't escape
    pitch.style.setProperty('overflow', 'hidden', 'important');

    // Inside pitch: make the layout + horizontal block fill the pitch
    var lw = pitch.querySelector('[data-testid="stLayoutWrapper"]');
    if(lw){
      lw.style.setProperty('flex', '1 1 0%', 'important');
      lw.style.setProperty('min-height', '0', 'important');
    }
    var hBlock = pitch.querySelector('[data-testid="stHorizontalBlock"]');
    if(hBlock){
      hBlock.style.setProperty('flex', '1 1 0%', 'important');
      hBlock.style.setProperty('align-items', 'stretch', 'important');
    }
    // Inner pitch columns: fill height + distribute players evenly
    if(hBlock){
      hBlock.querySelectorAll(':scope > [data-testid="stColumn"]').forEach(function(ic){
        ic.style.setProperty('display', 'flex', 'important');
        ic.style.setProperty('flex-direction', 'column', 'important');
        var vb = ic.querySelector('[data-testid="stVerticalBlock"]');
        if(vb){
          vb.style.setProperty('flex', '1 1 0%', 'important');
          vb.style.setProperty('justify-content', 'space-evenly', 'important');
        }
      });
    }
  }
  // Run after render + on mutations
  setTimeout(fill, 300);
  setTimeout(fill, 800);
  setTimeout(fill, 1500);
  var row = doc.querySelector('.st-key-pitch_row');
  if(row){
    var t = null;
